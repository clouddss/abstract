...
import { GetTokensParams, GetTokensResponse } from '@/lib/api/types/token.types'
...
export function useInfiniteTokens(params?: Omit<GetTokensParams, 'page'>) {
  ...
  const query = useInfiniteQuery<GetTokensResponse, Error>({
    queryKey: ['tokens', 'infinite', params],
    queryFn: ({ pageParam }) => 
      tokensService.getTokens({ ...params, page: pageParam as number, limit }),
...
export function useInfiniteTokensByCreator(creator: Address | undefined, limit: number = 20) {
